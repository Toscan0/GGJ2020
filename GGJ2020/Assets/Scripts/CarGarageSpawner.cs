...
public class CarGarageSpawner : MonoBehaviour
{
    ...
    void Start()
    {
        InvokeRepeating("SpawnCars", 5, 5);
    }

    void SpawnCars()
    {
        GameObject garage = garages[Random.Range(0, 4)];
        if (garage.name == "Cube" || garage.name == "Cube (1)" || garage.name == "Cube (2)")
        {
            car.transform.localScale = new Vector3(58f, 58f, 58f);
            GameObject aux = Instantiate(car, garage.transform.position, Quaternion.AngleAxis(-90, new Vector3(0, 1, 0)));
        }
        else
        {
            car.transform.localScale = new Vector3(58f, 58f, 58f);
